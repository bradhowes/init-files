...
(defun cmake-find-last-indented-line ()
  ...
  (when (re-search-backward "\w*[A-Za-z0-9_]+(" nil t)
    (back-to-indentation)))
...
(defun cmake-indent ()
  ...
  (unless (cmake-line-starts-inside-string)
    (if (bobp)
      ...
      (let (cur-indent ori-indent continuation-indent)
        (save-excursion
          (beginning-of-line)
          (let ((point-start (point))
            ...
            (setq ori-indent (current-indentation)
                  cur-indent nil
            ...
            (while (re-search-forward cmake-regex-token point-start t)
              ...
              ;; Remember the location of the '(' for continuing lines
              (when (string-match (concat "^" cmake-regex-paren-left "$") token)
                (setq continuation-indent (- (point) (point-at-bol))))

              ;; Indent due to blocks
              (when (and (string-match cmake-regex-block-open token)
                         (looking-at (concat "[ \t]*" cmake-regex-paren-left)))
                (setq cur-indent (+ ori-indent cmake-tab-width)))

              ;; Cancel any continuation indentation since it is closed
              (when (string-match (concat "^" cmake-regex-paren-right "$") token)
                (setq continuation-indent nil))
              )

            (goto-char point-start)

            ;; If this line closes the block, decrease indentation
            (if (looking-at cmake-regex-close)
                (setq cur-indent (- ori-indent cmake-tab-width)))
            )
          )

        ;; Set an indentation if not already set above.
        (if (not cur-indent)
            (if continuation-indent
                (setq cur-indent continuation-indent)
              (setq cur-indent ori-indent)))
